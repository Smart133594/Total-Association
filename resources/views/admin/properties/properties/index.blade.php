            ...
            <div class="col-md-12">
                ...
                <div class="ms-panel">
                    <div class="ms-panel-header ms-panel-custome">
                        ...
                        <a href="{{route('properties.create')}}" class="ms-text-primary">Add Properties </a>
                    </div>
                                ...
                                @if(!empty($alldata) &&  $alldata->count()>0)
                                    @foreach($alldata as $key=>$val)
                                        <tr role="row" class="odd">
                                            <td class="sorting_1" onclick="goto('/properties/{{ $val->edit_id }}')">{{$key+1}}</td>
                                            @if($setting['is_subassociations']=="1")
                                                <td onclick="goto('/properties/{{ $val->edit_id }}')">{{$val->name}}</td>
                                            @endif
                                            <td onclick="goto('/properties/{{ $val->edit_id }}')"> @if($val->buildingId>0){{$buld[$val->buildingId]}}@endif </td>
                                            <td onclick="goto('/properties/{{ $val->edit_id }}')">{{$val->aptNumber}}</td>
                                            <td onclick="goto('/properties/{{ $val->edit_id }}')">@if(isset($owner[$val->id])) {{$owner[$val->id]}} @else N/A @endif</td>
                                            <td onclick="goto('/properties/{{ $val->edit_id }}')">@if(isset($resident[$val->id])) {{$resident[$val->id]}} @else N/A @endif</td>
                                            <td onclick="goto('/properties/{{ $val->edit_id }}')"> @if($val->status==1)<span style="color: #13ce18">Current</span> @else <span style="color: #ff0000">Deliquent</span></i>@endif

                                            </td>
                                            <td onclick="goto('/properties/{{ $val->edit_id }}')">@if(isset($resident[$val->id])) <i class="fas fa-dot-circle dot-green"></i> @else <i
                                            ...
                                            <td class="action">
                                                ...
                                                <div class="dropdown show">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                                        <a class="dropdown-item" href="/properties/{{ $val->edit_id }}">See Info</a>
