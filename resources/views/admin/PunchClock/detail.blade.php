    ...
    <div class="ms-panel">
        ...
        <div class="ms-panel-body">
            ...
            <div style="padding:10px">
                <h6>Association: {{ @$punchClock->association }}</h6>
                <h6>Current State: {{ @$punchClock->state == 0 ? "Clocked In" : "Clocked Out" }}</h6>
                <p>{{ @$punchClock->note }}</p>
            </div>
            <hr> <br/>
            <div class="row">
                <div class="col-6">
                    <h5>Clock In</h5>
                    <hr>
                    <p>Serial Number: {{ @$punchClock->in_meta->AccessDevice->serial_number }}</p>
                    <p>IP Address: {{ @$punchClock->in_meta->AccessDevice->ip_address }}</p>
                    <p>Status: {{ @$punchClock->in_meta->AccessDevice->state == 0 ? "Disactive" : "Active" }}</p>
                    <p>Date: {{ @$punchClock->in_date }}</p>

                    @if (@$punchClock->in_meta)
                        <p>Location: {{ @$punchClock->in_meta->city }}, {{ @$punchClock->in_meta->area }}, {{ @$punchClock->in_meta->country }}</p>
                        <p>Postal code: {{ @$punchClock->in_meta->postal_code }}</p>
                    @endif
