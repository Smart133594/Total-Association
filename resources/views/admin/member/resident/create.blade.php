...
@section('content')
            ...
            <div class="col-md-12">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb pl-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page"><a href="{{route('resident.create')}}">@if($data ?? '') Update @else Add @endif</a></li>
                    </ol>
                </nav>
                <div class="ms-panel">
                    <div class="ms-panel-header ms-panel-custome">
                        <h6>@if($data ?? '') Update @else Add @endif Resident</h6>
                    </div>
                    <div class="ms-panel-body">
                        @include('admin.includes.msg')
                        <form id="admin-form" class="main-form" method="post" action="{{route('resident.store')}}" enctype="multipart/form-data">
                            @csrf
                            @if($data ?? '')
                            ...
                            <div class="row">
                                        ...
                                        <select type="text" class="form-control" id="propertyId" name="propertyId" required onchange="gettype()">
                                            ...
                                            @if($setting['is_subassociations']=="0" || isset($data))
                                                @foreach($property as $p)
                                                    <option value="{{$p->id}}" type="{{$p->type}}" owner="{{$p->owner}}"
                                                            @if($data ?? '') @if($data->propertyId==$p->id) selected @endif @endif>{{$p->type}}/{{$p->building}}/{{$p->aptNumber}}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div>
                                    <input type="hidden" name="ownerId" id="ownerId" @if($data ?? '')value="" @endif>
                                ...
                                {{--                                        <input type="radio" id="radio_1" name="isOwnerResident" onclick="iscompany(this.value)" value="0" @if(!isset($data)) checked @endif  @if($data ?? '') @if($data->isOwnerResident=='0')checked @endif @endif> <label for="radio_1" id="myradio_1">No</label>--}}
                                {{--                                        &nbsp;&nbsp;&nbsp;&nbsp;--}}
                                {{--                                        <input type="radio" id="isCompany" name="isOwnerResident" onclick="iscompany(this.value)" value="1" @if($data ?? '') @if($data->isOwnerResident=='1')checked @endif @endif> <label for="isCompany">Yes</label> &nbsp;&nbsp;&nbsp;&nbsp;--}}
                                {{--                                    </div>--}}
                                {{--                                </div>--}}
                            </div>
                            <div class="group_text">
                                <h2>Resident Basic Information </h2>
                            </div>
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control group_owner" id="firstName" name="firstName" value="@if($data ?? ''){{$data->firstName}}@endif" required>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group ">
                                        <label for="examplePassword">Middle Name</label>
                                        <input type="text" class="form-control group_owner" id="middleName" name="middleName" value="@if($data ?? ''){{$data->middleName}}@endif">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="examplePassword">Last Name</label>
                                        <input type="text" class="form-control group_owner" id="lastName" name="lastName" value="@if($data ?? ''){{$data->lastName}}@endif" required>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control group_owner" id="phoneNo" name="phoneNumber" value="@if($data ?? ''){{$data->phoneNumber}}@endif" required>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="examplePassword">Email Id</label>
                                        <input type="email" class="form-control group_owner" id="email" name="email" value="@if($data ?? ''){{$data->email}}@endif" required>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="examplePassword">WhatsApp Number</label>
                                        <input type="text" class="form-control group_owner" id="whatsapp" name="whatsapp" value="@if($data ?? ''){{$data->whatsapp}}@endif">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control group_owner" id="mailingAddress1" name="mailingAddress1" value="@if($data ?? ''){{$data->mailingAddress1}}@endif"
                                               required>
                                    </div>
                                </div>
                                <div class="col-md-8">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control group_owner" id="mailingAddress2" name="mailingAddress2" value="@if($data ?? ''){{$data->mailingAddress2}}@endif">
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control group_owner" id="city" name="city" value="@if($data ?? ''){{$data->city}}@endif" required>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="examplePassword">State</label>
                                        <input type="text" class="form-control group_owner" id="state" name="state" value="@if($data ?? ''){{$data->state}}@endif" required>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="examplePassword">Country</label>
                                        <input type="text" class="form-control group_owner" id="country" name="country" value="@if($data ?? ''){{$data->country}}@endif" required>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="examplePassword">Zip</label>
                                        <input type="text" class="form-control group_owner" id="zip" name="zip" value="@if($data ?? ''){{$data->zip}}@endif" required>
                                    </div>
                                </div>
                            </div>
                            <div class="group_text">
    ...
    <script>
        ...
        function uploaddoc() {
            $.get('/uploaddocresident/{{$ref}}', function (res) {
                $("#tbody").html(res);
            })
        }
        ...
        function iscompany(type) {
            if (type == 1) {
                ...
                if (propertyId) {
                    $.get('/get-ownerdetails/' + propertyId, function (res) {

                        $("#country").val(res.country);
                        $("#zip").val(res.zip);
                        $("#state").val(res.state);
                        $("#city").val(res.city);
                        $("#mailingAddress2").val(res.mailingAddress2);
                        $("#mailingAddress1").val(res.mailingAddress1);
                        ...
                        $(".group_owner").prop('readonly', true);
                    })

                } else {
                    alert("choose a property first");
                    $("#radio_1").prop("checked", true);
                }
            } else {
                $(".group_owner").val('');
                $(".group_owner").prop('readonly', false);
            }
        }
        ...
        function gettype() {
            ...
            $("#ownerId").val($("#propertyId  option:selected").attr('owner'))
        }
